...
$demandeactive=$user->affichedemandedactiveenseignat();
 if(isset($_SESSION)){
...
<div class="container">
 ...
 <header>
        ...
        <!--nav-->
          <nav>
            ...
            <li><a href="listedesujetpfa.php">sujet pfa</a></li>
                ...
                <li><a href="listedesujetpfe.php">PFE<font color="red"><?php if ($pro[0]>0){
  ...
  <li class="current">
      ...
      echo $a_result[0]; }?></font> </a></li>
                  <li><a class="subCurrent" href="demandedactiveenseignant.php">demande enseignant <font color="red"><?php if ($a_result1[0]>0){
 ...
 <?php foreach ($demandeactive as $key=>$value): ?>
  ...
  <tr><td>  <a href="enseignant.php?cin=<?php echo $value['cin']?>" target="_blank"><?php echo $value['nom']; ?> </td></a>
		<td><?php echo $value['prenom']; ?> </td>
		<td><?php   $sql="SELECT * FROM departement WHERE id='".$value['iddepartement']."'";
  $req = mysql_query($sql);
 $data= mysql_fetch_assoc($req);
  echo $data['nomdep'];                                ?> </td>
		<td><?php echo $value['grade']; ?> </td>
		<td><?php echo $value['email'] ; ?> </td><td>
     <a href="demandedactiveenseignant.php?cin=<?php echo $value['cin'];?>&email=<?php echo $value['email'];?>&active=oui"onclick="return(confirm('Etes-vous sûr de l active ?'));"><img src="../image/active.png" width="25" height="25"></a>

      <a href="demandedactiveenseignant.php?cin=<?php echo $value['cin'];?>&email=<?php echo $value['email'];?>&supp=delete"onclick="return(confirm('Etes-vous sûr de vouloir supprimer?'));"><img src="../image/nonactive.png" width="25" height="25"></a></td>
   </tr>
<?php endforeach ; }?>
</tbody>

    <?php if(isset($_GET['supp'])&& $_GET['supp']=='delete'){$user->suppdemandedactiveenseignant($_GET['cin'],$_GET['email']);} ?>
    <?php if(isset($_GET['active'])&& $_GET['active']=='oui'){$user->acceptedemandeactiveenseignant($_GET['cin'],$_GET['email']);} ?>
